...
export const useHiClawStore = create<HiClawState>()(
  persist(
    (set, get) => ({
      // Empty string means "use the server-side HICLAW_CONTROLLER_URL default".
      // This keeps the same dashboard image working in embedded (localhost)
      // and Kubernetes (in-cluster service) deployments.
      controllerUrl: process.env.NEXT_PUBLIC_HICLAW_CONTROLLER_URL || '',
      isConnected: false,
      ...
      checkConnection: async () => {
        ...
        try {
          const healthPath = url.trim()
            ? `/api/hiclaw/healthz/?controllerUrl=${encodeURIComponent(url)}`
            : '/api/hiclaw/healthz/';
    ...
    }),
    {
      ...
      migrate: (persistedState) => {
        ...
        // the server-side HICLAW_CONTROLLER_URL env var (works for embedded
        // localhost and Kubernetes in-cluster deployments).
        const s = persistedState as Partial<HiClawState>;
        if (s.controllerUrl === 'http://hiclaw-controller.hiclaw-system:8090') {
          s.controllerUrl = '';
        }
        return s as HiClawState;
      },
    }
  )
);
